...
function install_magisk() {
    ...
    if [ $ARCH = "arm64" ]; then
        wget -q https://storage.googleapis.com/magiskwaydroid/magiskarm64_0.8.tar.gz -O magisk.tar.gz
    else
        wget -q https://storage.googleapis.com/magiskwaydroid/magisk_0.8.4.tar.gz -O magisk.tar.gz
    fi
    sleep 0.3
    if [ $a = "2" ]; then
